...
#emotions.py imports cv2 numpy tensorflow utils and numpy
...
# get_labels function from utils dataset.py
...
emotion_classifier = load_model(emotion_model_path)   # trained model for emotions
...
while cap.isOpened(): # True: #ret for rectangle bgr_image for background
    ret, bgr_image = cap.read()


    gray_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2GRAY)     # gray image for HOGs
    
    rgb_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)       # color image for video frame red, green, blue
    ...
    for face_coordinates in faces:
        ...
        gray_face = np.expand_dims(gray_face, 0)          #adding 0 axis to the facial float 32-bit array, expand the shape of the array
        ...
        emotion_window.append(emotion_text)                             # add the label to emotion window
                                                                        # try test the block for errors and except handles the error
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):                            # exit conditions 255 hexa decimal
        break
